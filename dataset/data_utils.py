...
# class SimplePointcloudRandomCrop(object):
#     def __init__(self, crop_size=0.8, p=1):
...
#         x_min = np.random.uniform(0, 1 - self.crop_size)
#         y_min = np.random.uniform(0, 1 - self.crop_size)
#         z_min = np.random.uniform(0, 1 - self.crop_size)

#         x_max = x_min + self.crop_size
#         y_max = y_min + self.crop_size
#         z_max = z_min + self.crop_size

#         cropped_indices = np.where(
...
#         np.random.shuffle(cropped_indices)

#         # Randomly select a subset of cropped points
#         num_points_to_keep = np.random.randint(1, len(cropped_indices))
#         cropped_indices = cropped_indices[:num_points_to_keep]

#         return points[cropped_indices]
class SimplePointcloudRandomCrop(object):
    def __init__(self, crop_size=0.8, p=1):
        self.crop_size = crop_size
        self.p = p
    ...
    def __call__(self, points):
        ...
        # Filter points based on the crop size
        ...
        # Check if any points are cropped, otherwise return original points
        if len(cropped_points) == 0:
            return points

        return cropped_points.astype(np.float32)  # Ensure data type is consistent
...
class PointcloudRandomCrop(object):
    # def __init__(self, x_min=0.6, x_max=1.1, ar_min=0.75, ar_max=1.33, p=1, min_num_points=4096, max_try_num=10): 1212上的实验
    # def __init__(self, x_min=0.5, x_max=1.2, ar_min=0.7, ar_max=1.4, p=1, min_num_points=4096, max_try_num=2):
    # def __init__(self, x_min=0.5, x_max=1.2, ar_min=0.7, ar_max=1.4, p=1, min_num_points=4096, max_try_num=5):  0106 上的实验
    def __init__(self, x_min=0.5, x_max=1.2, ar_min=0.7, ar_max=1.4, p=1, min_num_points=4096, max_try_num=8):
        self.x_min = x_min
        self.x_max = x_max
...
class PointcloudRandomCrop1(object):
    def __init__(self, x_min=0.5, x_max=1.2, ar_min=0.7, ar_max=1.4, p=1, min_num_points=4096, max_try_num=5):
        self.x_min = x_min
        self.x_max = x_max
        self.ar_min = ar_min
        self.ar_max = ar_max
    ...
    def __call__(self, points):
        ...
        while not isvalid and try_num < self.max_try_num:
            new_coord_range = np.array([np.random.uniform(self.x_min, self.x_max),
                                        np.random.uniform(self.ar_min, self.ar_max),
                                        np.random.uniform(self.ar_min, self.ar_max)])
            
            new_coord_min = coord_min + coord_diff * np.random.uniform(0, 1 - new_coord_range)
            new_coord_max = new_coord_min + coord_diff * new_coord_range

            new_indices = (points[:, :3] > new_coord_min) & (points[:, :3] < new_coord_max)
            new_indices = np.all(new_indices, axis=1)
            new_points = points[new_indices]
...
class PointcloudRandomCrop2(object):
    def __init__(self, x_min=0.5, x_max=1.2, ar_min=0.7, ar_max=1.4, p=1, min_num_points=4096, max_try_num=5):
        self.x_min = x_min
        self.x_max = x_max
    ...
    def __call__(self, points):
        # 假设 points 已经是一个 GPU 上的 PyTorch 张量
        if torch.rand(1).item() > self.p:
            return points

        with torch.no_grad():  # 确保不记录这些操作的梯度
            ...
            while not isvalid and try_num < self.max_try_num:
                new_coord_range = torch.rand(3).to(points.device)  # 确保随机数在相同设备
                new_coord_range[0] *= (self.x_max - self.x_min)
                new_coord_range[0] += self.x_min
                new_coord_range[1:] *= (self.ar_max - self.ar_min)
                new_coord_range[1:] += self.ar_min

                new_coord_min = coord_min + coord_diff * torch.rand(3).to(points.device)
                new_coord_max = new_coord_min + coord_diff * new_coord_range

                new_indices = (points[:, :3] > new_coord_min) & (points[:, :3] < new_coord_max)
                new_indices = torch.all(new_indices, dim=1)
                new_points = points[new_indices]
            ...
            return new_points if isvalid else points
