"""MVMS-MAE augmentations: extends MVMS-v2 augmentations with MAE patch grouping support.

Point augmentations are identical to MVMS-v2 (clean_global, global, mid, local).
Image augmentations are identical to MVMS-v2 (weak_full, full, crop, hard).
New: MAE patch grouping utilities for masked autoencoding on point clouds.
"""
...
# --- MAE patch grouping utilities ---

def fps_torch(points: torch.Tensor, num_centers: int) -> torch.Tensor:
    """Farthest point sampling on GPU.
    points: [N, 3], returns indices [num_centers]
    """
    N = points.shape[0]
    if N <= num_centers:
    ...
    xyz = points[:, :3]
    selected = torch.zeros(num_centers, dtype=torch.long, device=points.device)
    distances = torch.full((N,), 1e10, device=points.device)
...
def knn_torch(query: torch.Tensor, reference: torch.Tensor, k: int) -> torch.Tensor:
    """K-nearest neighbors on GPU.
    query: [M, 3], reference: [N, 3] -> indices [M, k]
    """
    dist = torch.cdist(query[:, :3].unsqueeze(0), reference[:, :3].unsqueeze(0)).squeeze(0)  # [M, N]
    _, idx = dist.topk(k, dim=1, largest=False)  # [M, k]
    return idx


def group_points_with_centers(points: torch.Tensor, centers_idx: torch.Tensor, group_size: int) -> torch.Tensor:
    """Group points around centers using kNN.
    points: [N, 3], centers_idx: [G] -> grouped [G, group_size, 3]
    """
    centers = points[centers_idx]  # [G, 3]
    nn_idx = knn_torch(centers, points, group_size)  # [G, group_size]
    grouped = points[nn_idx]  # [G, group_size, 3]
    return grouped
...
def build_image_transform(kind: str, output_size: int = 224):
    """Build image transform for MVMS-MAE views (same as MVMS-v2)."""
