...
class PointNet(nn.Module):
    def __init__(self, args, output_channels=40,cls = -1):
        ...
        self.inv_head = nn.Sequential(
                            nn.Linear(args.emb_dims, args.emb_dims*2),
                            nn.BatchNorm1d(args.emb_dims*2),
                            nn.ReLU(inplace=True),
...
class PointNet1(nn.Module):
    def __init__(self, input_channels,  feature_transform=False):
    ...
    def forward(self, x):
        ...
        inv_feat = self.inv_head(feat)   #256
        inv_feat1 = self.inv_head1(feat) #128
        
        return x, inv_feat,inv_feat1,feat, T2
...
class DGCNNSemSeg(nn.Module):
    def __init__(self, emb_dims, k, dropout=0.5, num_channel=9, num_class=40, pretrain=False):
        ...
        self.inv_head2 = nn.Sequential(
            nn.Linear(emb_dims, emb_dims),
            nn.BatchNorm1d(emb_dims),
            nn.ReLU(inplace=True),
            nn.Linear(emb_dims, 256)
        )
    ...
    def forward(self, x):
        ...
        if self.pretrain:
            ...
            x = x.squeeze()
            inv_feat1 = self.inv_head1(x)
            inv_feat2 = self.inv_head2(x)

            return x, inv_feat1,inv_feat2, x
        else:
